...
@OnlyIn(Dist.CLIENT)
public class Roar extends TextureSheetParticle {
    public float r, g, b;
    ...
    public Roar(ClientLevel world, double x, double y, double z, double motionX, double motionY, double motionZ, float yaw, float pitch, int duration, float r, float g, float b, float opacity, float size, boolean facesCamera, EnumRingBehavior behavior, SpriteSet sprites) {
        super(world, x, y, z);
    ...
    public void render(VertexConsumer buffer, Camera renderInfo, float partialTicks) {
        ...
        buffer.vertex(avector3f[3].x(), avector3f[3].y(), avector3f[3].z()).uv(f8, f6).color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(j).endVertex();
        buffer.vertex(avector3f[2].x(), avector3f[2].y(), avector3f[2].z()).uv(f8, f5).color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(j).endVertex();
        buffer.vertex(avector3f[1].x(), avector3f[1].y(), avector3f[1].z()).uv(f7, f5).color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(j).endVertex();
        buffer.vertex(avector3f[0].x(), avector3f[0].y(), avector3f[0].z()).uv(f7, f6).color(this.rCol, this.gCol, this.bCol, this.alpha).uv2(j).endVertex();
    }
    ...
    public static final class RoarFactory implements ParticleProvider<RoarData> {
        ...
        public Particle createParticle(RoarData typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            Roar particle = new Roar(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, typeIn.getYaw(), typeIn.getPitch(), typeIn.getDuration(), typeIn.getR(), typeIn.getG(), typeIn.getB(), typeIn.getA(), typeIn.getScale(), typeIn.getFacesCamera(), typeIn.getBehavior(),spriteSet);
            return particle;
        }
    }

    public static class RoarData implements ParticleOptions {
        public static final Deserializer<RoarData> DESERIALIZER = new Deserializer<RoarData>() {
            public RoarData fromCommand(ParticleType<RoarData> particleTypeIn, StringReader reader) throws CommandSyntaxException {
                ...
                return new RoarData(yaw, pitch, duration, r, g, b, a, scale, facesCamera, EnumRingBehavior.GROW);
            }

            public RoarData fromNetwork(ParticleType<RoarData> particleTypeIn, FriendlyByteBuf buffer) {
                return new RoarData(buffer.readFloat(), buffer.readFloat(), buffer.readInt(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readBoolean(), EnumRingBehavior.GROW);
            }
        };
        ...
        public RoarData(float yaw, float pitch, int duration, float r, float g, float b, float a, float scale, boolean facesCamera, EnumRingBehavior behavior) {
            this.yaw = yaw;
            this.pitch = pitch;
            this.r = r;
            this.g = g;
        ...
        @Override
        public ParticleType<RoarData> getType() {
            return ModParticles.ROAR.get();
        }
        ...
        public static Codec<RoarData> CODEC(ParticleType<RoarData> particleType) {
            return RecordCodecBuilder.create((codecBuilder) -> codecBuilder.group(
                            Codec.FLOAT.fieldOf("yaw").forGetter(RoarData::getYaw),
                            Codec.FLOAT.fieldOf("pitch").forGetter(RoarData::getPitch),
                            Codec.FLOAT.fieldOf("r").forGetter(RoarData::getR),
                            Codec.FLOAT.fieldOf("g").forGetter(RoarData::getG),
                            Codec.FLOAT.fieldOf("b").forGetter(RoarData::getB),
                            Codec.FLOAT.fieldOf("a").forGetter(RoarData::getA),
                            Codec.FLOAT.fieldOf("scale").forGetter(RoarData::getScale),
                            Codec.INT.fieldOf("duration").forGetter(RoarData::getDuration),
                            Codec.BOOL.fieldOf("facesCamera").forGetter(RoarData::getFacesCamera),
                            ...
                            new RoarData(yaw, pitch, duration, r, g, b, a, scale, facesCamera, EnumRingBehavior.valueOf(behavior)))
            );
        }
    }
}