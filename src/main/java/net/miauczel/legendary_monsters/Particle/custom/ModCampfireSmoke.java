...
public class ModCampfireSmoke extends TextureSheetParticle {
    ModCampfireSmoke(ClientLevel pLevel, double pX, double pY, double pZ, double pXSpeed, double pYSpeed, double pZSpeed, boolean pSignal) {
        super(pLevel, pX, pY, pZ);
        this.scale(3.0F);
        this.setSize(0.25F, 0.25F);
        if (pSignal) {
            this.lifetime = this.random.nextInt(15) + 40;
        } else {
            this.lifetime = this.random.nextInt(15) + 10;
        }
    ...
    public static class CosyProvider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType pType, ClientLevel pLevel, double pX, double pY, double pZ, double pXSpeed, double pYSpeed, double pZSpeed) {
            ModCampfireSmoke campfiresmokeparticle = new ModCampfireSmoke(pLevel, pX, pY, pZ, pXSpeed, pYSpeed, pZSpeed, false);
    ...
    public static class SignalProvider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType pType, ClientLevel pLevel, double pX, double pY, double pZ, double pXSpeed, double pYSpeed, double pZSpeed) {
            ModCampfireSmoke campfiresmokeparticle = new ModCampfireSmoke(pLevel, pX, pY, pZ, pXSpeed, pYSpeed, pZSpeed, true);
