...
@OnlyIn(Dist.CLIENT)
public class BeheadedKnightSweepParticle extends TextureSheetParticle {
    private final SpriteSet sprites;
    public float r, g, b;
    public float yaw;
    public float pitch;

    public BeheadedKnightSweepParticle(ClientLevel pLevel, double pX, double pY, double pZ, SpriteSet spriteSet, double xd, double yd, double zd, float scale, float yaw, float pitch, float r, float g, float b) {
    ...
    public static class Provider implements ParticleProvider<SweepData> {
        ...
        public Particle createParticle(SweepData options, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            BeheadedKnightSweepParticle shriekparticle = new BeheadedKnightSweepParticle(worldIn, x, y, z, sprite, xSpeed, ySpeed, zSpeed, options.getScale(), options.getYaw(), options.getPitch(),options.r,options.g,options.b);
    ...
    public static class SweepData implements ParticleOptions {
        public static final Deserializer<SweepData> DESERIALIZER = new Deserializer<SweepData>() {
            public SweepData fromCommand(ParticleType<SweepData> particleTypeIn, StringReader reader) throws CommandSyntaxException {
                ...
                return new SweepData(scale, yaw, pitch,r,g,b);
            }

            public SweepData fromNetwork(ParticleType<SweepData> particleTypeIn, FriendlyByteBuf buffer) {
                return new SweepData(buffer.readFloat(), buffer.readFloat(), buffer.readFloat(),buffer.readFloat(),buffer.readFloat(),buffer.readFloat());
            }
        };
        ...
        public String writeToString() {
            return String.format(Locale.ROOT, "%s %.2f %.2f %.2f %.2f %.2f %.2f", BuiltInRegistries.PARTICLE_TYPE.getKey(this.getType()),
                    this.scale, this.yaw, this.pitch,this.r,this.g,this.b);
        }

        @Override
        public ParticleType<SweepData> getType() {
            return ModParticles.BEHEADED_KNIGHT_SWEEP.get();
        }
