...
public class ElectricityEntity extends Entity {
    ...
    private static final EntityDataAccessor<Float> DAMAGE = SynchedEntityData.defineId(ElectricityEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> TRANSPARENCY = SynchedEntityData.defineId(ElectricityEntity.class, EntityDataSerializers.INT);
    ...
    public ElectricityEntity(EntityType<? extends ElectricityEntity> type, double getX, double gety, double getz, double p_36821_, double p_36822_, double p_36823_, Level level, float Yrot) {
        this(type, level);
    ...
    public ElectricityEntity(LivingEntity p_36827_, double p_36828_, double p_36829_, double p_36830_, Level p_36831_, float damage, float Yrot, float life) {
        this(ModEntities.ELectr.get(), p_36827_.getX(), p_36827_.getY(), p_36827_.getZ(), p_36828_, p_36829_, p_36830_, p_36831_, Yrot);
    ...
    public ElectricityEntity(EntityType<? extends ElectricityEntity> type, LivingEntity p_36827_, double getX, double gety, double getz, double p_36821_, double p_36822_, double p_36823_, float damage, Level level) {
        this(type, level);
    ...
    private void damage(LivingEntity ImpactEntity) {
        ...
        if (ImpactEntity.isAlive() && !ImpactEntity.isInvulnerable() && ImpactEntity != livingentity) {
            if (this.tickCount % 5 == 0) {
                if (livingentity == null && !(ImpactEntity instanceof TamableAnimal animal && animal.getOwner() == getOwner())) {
                    ...
                    if (this.getOwner() instanceof Cloud_GolemEntity) {
                    ...
                    DamageSource damageSource = new DamageSource(this.level().registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.INDIRECT_MAGIC), livingentity);
                    if (this.getOwner() instanceof Cloud_GolemEntity && !(ImpactEntity instanceof TamableAnimal animal && animal.getOwner() == getOwner())) {
                        ImpactEntity.hurt(damageSource, getDamage());
                    } else {
                        if (!(ImpactEntity instanceof TamableAnimal animal && animal.getOwner() == getOwner())) ImpactEntity.hurt(damageSource, getDamage());
                    }
    ...
    public void tick() {
        ...
        for (LivingEntity livingentity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(0.5D, 4.0D, 0.75D))) {
        ...
        if (this.level().isClientSide) {
            if (tickCount % 2 == 0) {
                ...
                for (int i = 0; i < numberOfSkulls; i++) {
                    float angle = getYRot() + (i - (numberOfSkulls / 2)) * angleStep;
                    ...
                    double spawnY = this.getY() + 2;
                    double spawnZ = this.getZ() + vecZ * 1;
                    this.level().addAlwaysVisibleParticle(ModParticles.LB.get(), spawnX, spawnY, spawnZ, 0, 0, 0);
                }
            }
        }
        if (!this.leftOwner) {
