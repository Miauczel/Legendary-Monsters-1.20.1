...
public class SoulPillarEntity extends INoRendererEntity {
    ...
    private static final EntityDataAccessor<Integer> LIFE = SynchedEntityData.defineId(SoulPillarEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Float> DAMAGE = SynchedEntityData.defineId(SoulPillarEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Boolean> ATTACK = SynchedEntityData.defineId(SoulPillarEntity.class, EntityDataSerializers.BOOLEAN);

    private static final EntityDataAccessor<Boolean> IS_RED = SynchedEntityData.defineId(SoulPillarEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public SoulPillarEntity(EntityType<? extends SoulPillarEntity> p_i50170_1_, Level p_i50170_2_) {
    ...
    public SoulPillarEntity(Level worldIn, double x, double y, double z, float p_i47276_8_, int p_i47276_9_, LivingEntity casterIn, int lifeTicks, float damage, boolean isRed) {
        this(ModEntities.SOUL_PILLAR.get(), worldIn);
    ...
    public void tick() {
        super.tick();
        if (tickCount == 1) {
            level().addParticle(getRed() ? ModParticles.GROUNDSOUL_RED.get() : ModParticles.GROUNDSOUL.get(), getX(), (double) getY() + 2, getZ(), 0, 0, 0);
            this.level().addParticle(new Circle.RingData(0f, (float) Math.PI / 2f, 25, getRed() ? uR : 0f, getRed() ? uG : 0.9f, getRed() ? uB : 0.8f, 0.8f, 20f, false, Circle.EnumRingBehavior.SHRINK), getX(), getY() + 0.2f, getZ(), 0, 0, 0);
        }
        if (lifeTicks == 19) {
            if (!this.isSilent()) {
            ...
            if (this.clientSideAttackStarted) {
                ...
                if (lifeTicks == 19) {
                    for (int i = 0; i < 80; ++i) {
                        ...
                        smashParticles(this, 0, 1, 1, true);

                    }
                    this.level().addParticle(new Circle.RingData(0f, (float) Math.PI / 2f, 15, getRed() ? uR : 0f, getRed() ? uG : 0.9f, getRed() ? uB : 0.8f, 0.8f, 20f, false, Circle.EnumRingBehavior.GROW), getX(), getY() + 0.2f, getZ(), 0, 0, 0);

                    double d0 = this.getX();
                    double d1 = this.getY() + 2.0D;
                    double d2 = this.getZ();

                    this.level().addAlwaysVisibleParticle(getRed() ? ModParticles.SOULSTRIKE_RED.get() : ModParticles.SOULSTRIKE.get(), d0, d1, d2, 0, 0, 0);
                }
    ...
    public void smashParticles(Entity entity, float additionalY, float power, float r, boolean strict) {
        Level level = entity.level();
        Vec3 vec3 = new Vec3(entity.getX(), entity.getOnPos().getY(), entity.getZ());
        BlockPos pos = BlockUtils.blockPosVec3(vec3);
        BlockParticleOption blockparticleoption = new BlockParticleOption(ModParticles.DUST_FALLING.get(), level.getBlockState(pos));
                ...
                if (livingentity == null) {
                    ...
                    if (this.getCaster() instanceof Frostbitten_GolemEntity) {
                        ...
                        if (flag) {
                            ...
                            EntityUtil.applyStackingEffect((LivingEntity) ImpactEntity, ModEffects.SOUL_FRACTURE.get(), 1, 4, MathUtils.toTicks(10));
    ...
    static {
        ATTACK_STATE = SynchedEntityData.defineId(SoulPillarEntity.class, EntityDataSerializers.INT);
    }
    ...
    public void onAddedToWorld() {
        ...
        //  this.level().addAlwaysVisibleParticle(ModParticles.SOULSTRIKE.get(), getX(), getY() + 2, getZ(), 0, 0, 0);
