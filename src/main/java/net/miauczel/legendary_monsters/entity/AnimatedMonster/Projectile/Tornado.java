...
public class Tornado extends Entity {
    ...
    private static final EntityDataAccessor<Float> DAMAGE = SynchedEntityData.defineId(Tornado.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> TRANSPARENCY  = SynchedEntityData.defineId(Tornado.class, EntityDataSerializers.INT);
    ...
    public Tornado(EntityType<? extends Tornado> type, double getX, double gety, double getz, double p_36821_, double p_36822_, double p_36823_, Level level,float Yrot) {
        this(type, level);
    ...
    public Tornado(LivingEntity p_36827_, double p_36828_, double p_36829_, double p_36830_, Level p_36831_, float damage,float Yrot,float life) {
        this(ModEntities.TORNADO.get(), p_36827_.getX(), p_36827_.getY(), p_36827_.getZ(), p_36828_, p_36829_, p_36830_, p_36831_,Yrot);
    ...
    public Tornado(EntityType<? extends Tornado> type, LivingEntity p_36827_, double getX, double gety, double getz, double p_36821_, double p_36822_, double p_36823_, float damage, Level level) {
        this(type, level);
            ...
            if (this.tickCount % 5 == 0) {
                if (livingentity == null) {
                    ...
                    if (this.getOwner() instanceof Cloud_GolemEntity) {
                        ImpactEntity.hurt(damageSource, getDamage());
                    }else {ImpactEntity.hurt(damageSource, getDamage());}
    ...
    public void tick() {
        ...
        DamageAndGrab();
        List<LivingEntity> entities = new ArrayList<>(this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(0.75, getBbHeight(), 0.75), EntitySelector.NO_CREATIVE_OR_SPECTATOR));
for (LivingEntity entity :entities){
    if (entity.isShiftKeyDown()) {
        ...
        ;if (!hasHurt) {
            this.setDeltaMovement(vec3.add(this.xPower, 0, this.zPower).scale((double) f));
        }
    ...
    public void grab() {
        ...
        for (LivingEntity livingentity : entities) {
            if (this.getOwner() != null) {
                boolean hurt = livingentity.hurt(damageSources().mobAttack(this.getOwner()), 1);
                if (hurt) {
                    if (!hasHurt) {
                        hasHurt = true;
                                    ...
                                    if (((Mob) livingonwer).getTarget() != null) {
                                        ...
                                        if (livingentity instanceof Player player) {
                                            ...
                                            if (!livingentity.isBlocking()) {
                                                ...
                                                if (livingentity.hurt(damageSources().mobProjectile(this, livingonwer), getDamage())) {
                                                }
                                            }
                                        } else {
                                            if (livingentity.hurt(damageSources().mobProjectile(this, livingonwer), getDamage())) {
