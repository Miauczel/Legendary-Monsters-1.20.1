...
public class FlamebornGuardBlockGoal extends Goal {
    protected final FlamebornGuardEntity entity;
    ...
    public FlamebornGuardBlockGoal(FlamebornGuardEntity entity, int getattackstate, int attackstate, int attackendstate, int attackfinaltick, int attackseetick) {
        this.entity = entity;
    ...
    public FlamebornGuardBlockGoal(FlamebornGuardEntity entity, int getattackstate, int attackstate, int attackendstate, int attackfinaltick, int attackseetick, boolean interruptsAI) {
        this.entity = entity;
        if (interruptsAI) {
            this.setFlags(EnumSet.of(Goal.Flag.MOVE, Goal.Flag.LOOK, Goal.Flag.JUMP));
        }
    ...
    public void tick() {
        ...
        if (attackTicks == 11){
            if (target !=null) {
                ...
                float vec = -4;
                float offset = 0;
                entity.teleport((target.getX() + vec * vecX + f * offset),entity.getY(),(target.getZ() + vec * vecZ + f1 * offset));
            }
        }
    }
