...
@OnlyIn(Dist.CLIENT)
public class FracturedApostleKnifeLayer extends RenderLayer<FracturedApostleEntity, FracturedApostleModel<FracturedApostleEntity>> {
    private static final ResourceLocation BODY = new ResourceLocation(LegendaryMonsters.MOD_ID, "textures/entity/fractured_apostle/fractured_apostle_knife.png");


    public FracturedApostleKnifeLayer(FracturedApostleRenderer renderIn) {
        super(renderIn);
    }
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, FracturedApostleEntity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        float animationProgress1 = Math.min(entity.weaponFade.getAnimationFraction(), 1F);
        float f_0_245976 = 1f - animationProgress1;
        // System.out.println("f: " + f_0_245976);
        if (entity.hasGhostKnife()) {
